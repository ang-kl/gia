...
- 11. Try-line discovery rule (v0.62.29): the card's 🍲 Try dish is review-mined FIRST (cuisine-enrich extractDishes); when reviews yield nothing on a single-cuisine search, a curated national/iconic dish (NATION_OVERLAY, with COOKING_METHODS as a secondary term) fills the line ONLY if the venue's OWN reviews / editorial summary mention it — evidence-verified, never a blind claim. The unfamiliar-family case (cuisine family not local to the search country, e.g. European searched from SG) is the foodie-discovery emphasis; logged as D786. Special modes + Michelin excluded.
