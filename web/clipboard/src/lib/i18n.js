...
const STRINGS = {
  ...
  'chrome.pickLocal':       { en: '📍 Pick Local Dish', fr: '📍 Plat local', id: '📍 Hidangan lokal', ru: '📍 Местное блюдо', de: '📍 Lokales Gericht' , zh: '📍 选本地美食', ja: '📍 ローカル料理を選ぶ', es: '📍 Elegir plato local' },
  ...
  'filter.noDishes':        { en: 'No notable dishes in your saved cards yet.', fr: 'Aucun plat notable dans vos cartes.', id: 'Belum ada hidangan menonjol.', ru: 'Пока нет заметных блюд.', de: 'Noch keine besonderen Gerichte.' , zh: '你收藏的卡片里还没有招牌菜。', ja: '保存カードに注目の料理がまだありません。', es: 'Aun no hay platos destacados en tus tarjetas guardadas.' },
  'classic.tapHint':        { en: 'Tap a saved dish to filter', fr: 'Touchez un plat enregistré', id: 'Ketuk hidangan tersimpan', ru: 'Нажмите сохранённое блюдо', de: 'Gespeichertes Gericht tippen' , zh: '点一道收藏的菜来筛选', ja: '保存した料理をタップして絞り込み', es: 'Pulsa un plato guardado para filtrar' },
  'classic.more':           { en: 'More {city} classics ({n})', fr: 'Plus de classiques de {city} ({n})', id: 'Lebih banyak klasik {city} ({n})', ru: 'Ещё классика {city} ({n})', de: 'Mehr Klassiker aus {city} ({n})' , zh: '更多{city}经典 ({n})', ja: '{city}の定番をもっと見る（{n}）', es: 'Mas clasicos de {city} ({n})' },
  ...
  'set.privacyNote':        { en: 'Your cabinets and cards are stored against your Telegram account only, and expire per the limits above.', fr: 'Vos classeurs et cartes sont liés à votre compte Telegram et expirent selon les limites ci-dessus.', id: 'Kabinet & kartu Anda tersimpan untuk akun Telegram Anda saja, dan kedaluwarsa sesuai batas di atas.', ru: 'Ваши папки и карточки хранятся только для вашего аккаунта Telegram и истекают по лимитам выше.', de: 'Cabinets und Karten sind nur mit deinem Telegram-Konto gespeichert und verfallen gemäß den Limits oben.' , zh: '你的柜子和卡片仅存储在你的 Telegram 账户下，并按上述期限到期。', ja: 'キャビネットとカードはお客様のTelegramアカウントにのみ保存され、上記の制限に従って期限切れになります。', es: 'Tus armarios y tarjetas se guardan solo asociados a tu cuenta de Telegram y caducan segun los limites indicados arriba.' },
  ...
  'root.capCabinets':       { en: 'Cabinet cap reached ({cap}). Delete one first.', fr: 'Limite de classeurs atteinte ({cap}). Supprimez-en un.', id: 'Batas kabinet tercapai ({cap}). Hapus salah satu.', ru: 'Лимит папок ({cap}). Удалите одну.', de: 'Cabinet-Limit erreicht ({cap}). Lösche zuerst eines.' , zh: '已达柜子上限 ({cap})。请先删除一个。', ja: 'キャビネットの上限に達しました（{cap}）。まず1つ削除してください。', es: 'Limite de armarios alcanzado ({cap}). Elimina uno primero.' },
