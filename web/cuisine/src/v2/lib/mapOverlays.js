...
// v0.62.130 — exported so the venue pop-up's transit block can colour its
// station code chips by line (operator).
...
export function infoCard(inner, gmaps) {
  ...
  // — a raised, beveled card: top-light→bottom-shade gradient, a soft drop
  // shadow, a hairline border and an inset top highlight so it reads as a
  // physical card lifted off the map.
...
export function createOverlayController(map, googleMaps, opts) {
  ...
  function hawkerCardHtml(f, transit) {
    ...
    if (f.status) {
      ...
      h += '<div style="color:' + p.sub + ';margin-top:2px;">' + escapeHtml(scLabel('stalls', _lang, { n: f.stalls })) + '</div>';
    }
    if (f.address) {
