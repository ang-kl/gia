 ...
 */
export function secondLine({ primary, english, code, lang, say }) {
  ...
  // 2. A translation we authored for this locale.
  const local = code ? lineNameLocal(code, lang) : null;
  if (local && local !== english) return { text: `(${local})`, key: 'translated' };

  // 3. Failing that, how to SAY the English name. Never alongside the above —
  //    that would be a third language on one card.
