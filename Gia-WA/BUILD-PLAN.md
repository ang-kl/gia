...
4. Respond with AES-GCM-encrypted JSON using the **flipped IV** (XOR every byte of the request IV with `0xFF`). This is the bug that catches everyone the first time; if you reuse the request IV verbatim, Meta returns a vague 4xx and your Flow appears "stuck".
...
- **`src/flows/crypto.js`** — pure functions: `decryptRequest({ encrypted_aes_key, encrypted_flow_data, initial_vector }, privateKeyPem)` → `{ action, screen, data }`. `encryptResponse(payload, aesKey, requestIV)` → `{ encrypted_flow_data }`. **Flipped-IV is hard-coded inside `encryptResponse` to prevent the day-1 bug.**
