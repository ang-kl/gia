...
const LRT_CCL_STATIONS = [
  ...
  { name: 'Keppel',            lat: 1.2705, lng: 103.8266, codes: ['CC30'], lines: ['CCL'], status: 'future' },
  { name: 'Cantonment',        lat: 1.2741, lng: 103.8359, codes: ['CC31'], lines: ['CCL'], status: 'future' },
  { name: 'Prince Edward Road',lat: 1.2767, lng: 103.8450, codes: ['CC32'], lines: ['CCL'], status: 'future' },
...
describe('buildLinePaths — Circle Line closed loop (v0.61.1)', () => {
  it('draws CCL as one closed ring through the CCL6 stations, no detached CE spur', () => {
    ...
    expect(ring).toContainEqual({ lat: 1.2705, lng: 103.8266 }); // Keppel (CCL6)
    expect(ring).toContainEqual({ lat: 1.2741, lng: 103.8359 }); // Cantonment (CCL6)
    expect(ring).toContainEqual({ lat: 1.2767, lng: 103.8450 }); // Prince Edward Road (CCL6)
